    ...
    <Container ref={ref} style={containerStyle} className={insertFocus} >
      ...
      <ContainerInput>
        <TextInput
          ...
          onFocus={() => setInsertFocus('getFocus')}
          onBlur={() => setInsertFocus('')}
          {...rest}
        />
        ...
        {passwordView && (
          ...
          <ButtonEyeView type="button" onClick={() => setPasswordViewType(passwordViewType === 'password' ? 'text' : 'password')}>
            ...
            {passwordViewType === 'password' ? (
              <Eye size={16} color="#838383" />
            ) : (
              <EyeSlash size={16} color="#838383" />
            )}
          </ButtonEyeView>
        )}

        {error && <ErrorSymbol> <WarningCircle size={22} /> </ErrorSymbol>}
